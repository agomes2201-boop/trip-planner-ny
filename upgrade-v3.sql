...
ALTER TABLE attractions ADD COLUMN amount_usd DECIMAL(10,2) UNSIGNED NOT NULL DEFAULT 0 AFTER attraction_date;
ALTER TABLE attractions ADD COLUMN completed TINYINT(1) NOT NULL DEFAULT 0 AFTER amount_usd;
...
INSERT INTO expenses (city_id,name,category,amount_usd,expense_date,status,notes,attraction_id)
SELECT city_id,CONCAT('Passeio: ',name),'Passeios e ingressos',amount_usd,COALESCE(attraction_date,CURDATE()),'planned','Gerado automaticamente pelo cadastro de passeios.',id
FROM attractions WHERE amount_usd>0
ON DUPLICATE KEY UPDATE name=VALUES(name),amount_usd=VALUES(amount_usd),expense_date=VALUES(expense_date);
