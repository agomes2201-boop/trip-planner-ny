...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'delete' && $id) {
        ...
        $amount = (float)str_replace(',', '.', trim((string)($_POST['amount_usd'] ?? '0')));
        $date = ($_POST['attraction_date'] ?? '') ?: null;
        $data = [':city_id'=>$cityId, ':name'=>trim((string)($_POST['name'] ?? '')), ':description'=>trim((string)($_POST['description'] ?? '')), ':maps_url'=>trim((string)($_POST['maps_url'] ?? '')), ':attraction_date'=>$date, ':amount_usd'=>$amount, ':sort_order'=>(int)($_POST['sort_order'] ?? 0)];
        $pdo->beginTransaction();
        if ($id) { $data[':id']=$id; $pdo->prepare('UPDATE attractions SET city_id=:city_id,name=:name,description=:description,maps_url=:maps_url,attraction_date=:attraction_date,amount_usd=:amount_usd,sort_order=:sort_order WHERE id=:id')->execute($data); }
        else { $pdo->prepare('INSERT INTO attractions(city_id,name,description,maps_url,attraction_date,amount_usd,sort_order) VALUES(:city_id,:name,:description,:maps_url,:attraction_date,:amount_usd,:sort_order)')->execute($data); $id=(int)$pdo->lastInsertId(); }
        if ($amount > 0) {
            $stmt=$pdo->prepare("INSERT INTO expenses(city_id,name,category,amount_usd,expense_date,status,notes,attraction_id) VALUES(?,?,'Passeios e ingressos',?,?,'planned','Gerado automaticamente pelo cadastro de passeios.',?) ON DUPLICATE KEY UPDATE city_id=VALUES(city_id),name=VALUES(name),amount_usd=VALUES(amount_usd),expense_date=VALUES(expense_date)");
...
$edit ??= ['id'=>'','city_id'=>$cityId,'name'=>'','description'=>'','maps_url'=>'','attraction_date'=>'','amount_usd'=>'','completed'=>0,'sort_order'=>0];
?>
