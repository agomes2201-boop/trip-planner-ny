...
$totals = $pdo->query("SELECT status, COALESCE(SUM(amount_eur),0) total FROM expenses GROUP BY status")->fetchAll(PDO::FETCH_KEY_PAIR);
$byCity = $pdo->query("SELECT c.name, SUM(CASE WHEN e.status='paid' THEN e.amount_eur ELSE 0 END) paid, SUM(CASE WHEN e.status='planned' THEN e.amount_eur ELSE 0 END) planned FROM cities c LEFT JOIN expenses e ON e.city_id=c.id GROUP BY c.id,c.name HAVING paid>0 OR planned>0 ORDER BY c.sort_order")->fetchAll();
$byCategory = $pdo->query("SELECT category, SUM(CASE WHEN status='paid' THEN amount_eur ELSE 0 END) paid, SUM(CASE WHEN status='planned' THEN amount_eur ELSE 0 END) planned FROM expenses GROUP BY category ORDER BY paid DESC,planned DESC")->fetchAll();
